...
function normalizeActiveImplementation(value) {
  ...
  const retention = IMPLEMENTATION_PLAN_RETENTIONS.includes(value.retention) ? value.retention : "keep";
    ...
    result = await ctx.newSession({
      ...
      withSession: async (replacementCtx) => {
        ...
        if (pendingImplementationRuntime) {
          const runtimeStatus = destinationRuntimeConsumptionStatus(replacementCtx, request.stateEntryType);
...
// src/review-bridge.ts
import { randomUUID as randomUUID2 } from "node:crypto";
import { tmpdir } from "node:os";
import { join } from "node:path";

// src/preservation-policy.ts
import { lstatSync } from "node:fs";
import { dirname, isAbsolute, relative, resolve, sep } from "node:path";
function isScratchPath(cwd, input, scratchRoot) {
  ...
  if (process.platform === "win32" && local.split(/[\\/]/).some((part) => /[:]|[. ]$/.test(part) || /^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part)))
    return false;
...
function classifyPreservingCommand(shell, command, cwd, scratchRoot, safe = {}) {
  ...
  const executesCode = /\b(?:npm|npx|pnpm|yarn|node|python\d*|cargo|dotnet|make|pytest|vitest|jest|go|tsc)(?:\.exe)?\b/i.test(command);
  return inspection === "allow" && !executesCode ? "allow" : "review";
}

// src/review-bridge.ts
function registerReviewPolicy(pi, getState) {
  let session;
  let generation;
  let root = "";
  const scratchRoot = () => {
    ...
    if (!root || state.session !== session || state.generation !== generation) {
      ...
      root = join(tmpdir(), "pi-plan", randomUUID2());
    }
    return root;
  };
  pi.events.on("tool-review:policy:v2", (value) => {
    ...
    let inspection = "review";
    if (shell === "bash" || shell === "powershell") {
      inspection = classifyPreservingCommand(shell, readCommand(query.input), query.cwd, root2, state.safeSubcommands);
      if (!state.enabled && inspection === "deny") inspection = "review";
    }
    if (state.enabled && ["edit", "write"].includes(shell ?? "") && !isScratchPath(query.cwd, query.input?.path, root2))
      inspection = "deny";
    ...
    query.answers.push({
      ...
      inspection,
      scratchRoot: state.enabled ? root2 : void 0,
      revision: JSON.stringify([state.generation, state.safeSubcommands])
    });
  });
  pi.events.on("tool-review:policy:v1", (value) => {
    if (!value || typeof value !== "object") return;
    const query = value;
    const state = getState();
    if (query.session !== state.session || !Array.isArray(query.answers) || typeof query.cwd !== "string") return;
  ...
  const available = () => {
    const query = { session: getState().session, answers: [] };
    pi.events.emit("tool-review:capabilities:v2", query);
    return query.answers.length === 1;
  };
  return { scratchRoot, available };
}
async function requestInspectionReview(pi, event, ctx) {
  const query = { event, ctx, replies: [] };
...
function planMode(pi, dependencies = {}) {
  ...
  const loadInteractiveUi = () => {
    ...
    if (!interactiveUiPromise) {
      interactiveUiPromise = import("./chunks/interactive-ui-V3WYU5TL.js").catch((error) => {
  ...
  const startPlanModeSettingsWatch = (generation) => {
    ...
    try {
      const directory = dirname2(pathToWatch);
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (["bash", "powershell", "write", "edit"].includes(event.toolName)) {
      ...
      const blocked = shell ? (event.toolName === "bash" ? findBlockedCommandSegment : findBlockedPowerShellCommandSegment)(
      ...
      if (currentSession !== ctx.sessionManager || generation !== workflowGeneration || !state.enabled || !workflowMutex.isOwner(workflowOwner)) {
        return { block: true, reason: "Plan review cancelled because the workflow changed." };
      }
      if (result.allowed) return;
      if (blocked === void 0 && result.kind === "unavailable") return;
  ...
  pi.on("context", async (event, ctx) => {
    ...
    const withScratch = state.enabled && reviewPolicy.available() ? [
      ...messages,
      {
        role: "custom",
...
${intent.plan}`,
  ...
  function selectableTools() {
    return safeGetAllTools().filter((tool) => tool.name !== PLAN_MODE_QUESTION_TOOL_NAME && tool.name !== PLAN_MODE_COMPLETE_TOOL_NAME).sort(compareTools);
  }
