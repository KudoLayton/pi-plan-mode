...
function messageTextContent(message) {
  return messageText(message?.message ?? message);
}
...
var IMPLEMENTATION_THINKING_LEVELS = ["off", "minimal", "low", "medium", "high", "xhigh", "max"];
var IMPLEMENTATION_PLAN_RETENTIONS = ["clear-on-start", "clear-after-first-run", "keep"];
...
function normalizePlanModeSettings(value) {
  ...
  if (Object.hasOwn(value, "implementationPlanRetention")) {
    ...
    if (!IMPLEMENTATION_PLAN_RETENTIONS.includes(implementationPlanRetention)) {
      return void 0;
    }
  ...
  if (Object.hasOwn(value, "defaultImplementationThinkingLevel")) {
    ...
    if (!IMPLEMENTATION_THINKING_LEVELS.includes(defaultImplementationThinkingLevel)) {
      return void 0;
    }
...
function normalizeSafeSubcommands(value) {
  ...
  for (const [command, subcommands] of Object.entries(value)) {
    ...
    if (!normalizedCommand || !Array.isArray(subcommands) || !subcommands.every((item) => typeof item === "string" && item.trim().length > 0)) {
      return void 0;
    }
...
function createImplementationRetentionCoordinator() {
  ...
  return {
    ...
    transformContext(messages, state) {
      ...
      const filteredMessages = inactiveMessages.filter((message) => message === historyArtifactMessage || !messageContainsInactivePlanModeArtifact(message)).map(
...
function planModeQuestionAnswered(questions, answers) {
  return {
    content: [{ type: "text", text: formatPlanModeQuestionPayload({ cancelled: false, answers }) }],
...
var SAFE_GIT_SUBCOMMANDS = [...BUILTIN_SAFE_GIT_SUBCOMMANDS, ...CONFIGURABLE_SAFE_GIT_SUBCOMMANDS];
var SAFE_BUILTIN_PLAN_TOOLS = /* @__PURE__ */ new Set(["read", "bash", "powershell", "grep", "find", "ls"]);
var REVIEWED_FILE_TOOLS = /* @__PURE__ */ new Set(["edit", "write"]);
...
function isSafeCommand(command, safeSubcommands = {}, workingDirectory, platform = process.platform) {
  return findBlockedCommandSegment(command, safeSubcommands, workingDirectory, platform) === void 0;
}
...
function isSafePowerShellCommand(command, safeSubcommands = {}, workingDirectory) {
  return findBlockedPowerShellCommandSegment(command, safeSubcommands, workingDirectory) === void 0;
}
function classifyInspectionCommand(shell, command, safeSubcommands = {}, workingDirectory) {
  ...
  for (const segment of segments) {
    ...
    if (MUTATING_COMMANDS.has(name) || READ_ONLY_COMMANDS.has(name) || READ_ONLY_POWERSHELL_COMMANDS.has(name))
      return "deny";
    if (/^(git|gh|hostname|tasklist|get-process|get-service|bash|sh|zsh|cmd|pwsh|powershell|python\d*|node|eval|source|iex|invoke-expression|del|erase|rd|md|ren|move|copy|xcopy|robocopy|reg|icacls|cacls|format|diskpart|ac|clc|cli|clp|cpi|cpp|mi|mp|ni|ri|rni|rnp|si|sp|sc|npm|npx|pnpm|yarn|pip|uv|cargo|dotnet|make)$/.test(
      name
    ))
      return "deny";
    if (/^(set|new|remove|clear|add|rename|move|copy|start|stop|restart|install|uninstall|enable|disable|invoke|out|export|save|update|register|unregister)-/.test(
      name
    ))
      return "deny";
    if (words?.some(
      (word) => word === "--%" || word.startsWith("-") && /^(?:-out(file|variable)|-errorvariable|-warningvariable|-pipelinevariable|--output)(?:[=:]|$)/i.test(
        word
      )
    ))
      return "deny";
...
function hasSafeArguments(command, args) {
  ...
  if (command === "find" && args.some(
    (argument) => ["-exec", "-execdir", "-ok", "-okdir", "-fprint", "-fprint0", "-fprintf", "-fls"].includes(argument)
  )) {
    return false;
  }
  ...
  if (command === "fd" && args.some(
    (argument) => ["-x", "-X", "--exec", "--exec-batch"].some((flag) => argument === flag || argument.startsWith(`${flag}=`))
  )) {
    return false;
  }
...
function parseGitGlobalOptions(args, workingDirectory) {
  ...
  while (index < args.length) {
    ...
    if (argument === "-c") {
      ...
      if (safeDirectory !== void 0 || !setting?.startsWith("safe.directory=")) return void 0;
      safeDirectory = setting.slice("safe.directory=".length);
      if (!isAbsolute(safeDirectory) || /[*?]/u.test(safeDirectory)) return void 0;
      index += 2;
      continue;
    }
    if (argument !== "-C") break;
    const target = args[index + 1];
    if (!target || target.startsWith("-") || !directory || !workingDirectory) return void 0;
    if (target.split(/[\\/]+/u).includes("..")) return void 0;
    directory = resolve2(directory, target);
    const fromRoot = relative(workingDirectory, directory);
    if (isAbsolute(fromRoot) || fromRoot === ".." || fromRoot.startsWith(`..${sep}`)) return void 0;
    index += 2;
  }
  if (safeDirectory !== void 0 && (!directory || relative(directory, safeDirectory) !== "")) return void 0;
  return index;
}
...
export {
  ...
  classifyInspectionCommand,
  compareTools,
  toolPolicyLabel,
  defaultPlanModeToolNames,
  snapshotPlanModeSelectedNames
};
//# sourceMappingURL=chunk-MVY3NI7D.js.map
