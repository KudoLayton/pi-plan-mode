  ...
  return {
    settingsScreen: () => ({
      ...
      lines: [
        ...
        ...unavailableDefaultActive && configuredModel ? [`Configured default ${safeModelReference(configuredModel)} is unavailable; using same as plan.`] : []
      ],
      items: [
        {
          id: "start-fresh",
    ...
    start(signal) {
      if (selectedModelUsesDefault && selectedModel && !findAvailableImplementationModel(snapshotAvailableImplementationModels(ctx), selectedModel.model)) {
...
import { defineMenu as defineMenu5, runMenu as runMenu5, sanitizeTerminalText as sanitizeTerminalText2 } from "@narumitw/pi-tui-kit";
async function showPlanModeSettings(ctx, options) {
  ...
  const modelsByItemId = new Map(implementationModels.map((model) => [modelItemIds.get(model), model]));
  ...
  const menu = defineMenu5({
    ...
    actions: {
      ...
      "set-implementation-thinking": async ({ ctx: actionCtx, value, signal }) => {
        ...
        if (!IMPLEMENTATION_THINKING_LEVELS.includes(value)) {
          return { kind: "rejected" };
        }
        return savePatch(
          actionCtx,
...
//# sourceMappingURL=interactive-ui-V3WYU5TL.js.map
